...
def compute_torsion(coordinates, i, j, k, l):
    ...
    theta = np.arccos(cos_theta) * np.sign(
        np.dot(rkj, np.cross(t, u)).value_in_unit(nanometer**-5)
    )
...
def get_name(element_idx, table=None, pdb_file=None, format="full"):
    ...
    if format == "full":
        return (
            table.values[element_idx][4]
            + str(table.values[element_idx][3])
            + table.values[element_idx][2]
        )
    elif format == "short":
...
def get_name_torsion(indices, table=None, pdb_file=None, format="full"):
    ...
    if format == "full":
        return (
            get_name(indices[0], pdb_file=pdb_file, table=table)
            + "-"
        ...
        return (
            get_name(indices[0], pdb_file=pdb_file, table=table, format="short")
            + "-"
...
def correlations_features(
    evec, traj, list_of_functions, list_of_params=None, progress_bar=True
):
    """
    ...
    if list_of_params is None:
        for funcs in tqdm(list_of_functions):
            correlations.append(compute_correlation(evec, eval(funcs + "(traj)")))

    else:
        for funcs, pars in tqdm(zip(list_of_functions, list_of_params)):
...
def correlations_cv_torsion_index(evec, traj, list_of_indices):
    ...
    correlations = []
    indices_cv = []
    for indx in tqdm(list_of_indices):
...
def create_torsions_list(atoms, size=None, print_list=True, append_to=None):
    ...
    if size is None:
        ...
        chosen_indices = np.random.choice(
            range(len(all_possible_combinations)), size=size
        )

    if append_to is not None:
        all_combinations = np.vstack(
            (append_to, all_possible_combinations[chosen_indices])
        )
    else:
...
def compute_all_correlations(
    ...
    for ev_idx in range(dimension):
        ...
        correlations[ev_idx] = correlations_features(
            evec,
            traj[::nevery],
            list_of_functions,
...
def identify_features(correlations, all_combinations, remove_nans=False):
    ...
    for ev_idx in range(dimension):
        ...
        if remove_nans:
            correlations_clean_idx = np.array(
                [
                    i
                    for i in range(len(correlations[ev_idx]))
                    if str(correlations[ev_idx][i]) != "nan"
                ]
            )
        else:
...
def identify_worst_features(corr, all_combinations, dimension):
    ...
    while len(inter) < dimension and eps < 1:
        inter = np.intersect1d(
            np.where(np.abs(corr_arr[:, 0]) < eps)[0],
            np.where(np.abs(corr_arr[:, 1]) < eps)[0],
        )
        print(inter)
        eps += 0.01
    ...
    correlations_cv_minimal = np.array((corr[0], corr[1])).T[inter]
