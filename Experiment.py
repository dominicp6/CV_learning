...
class Experiment:
    ...
    def __init__(self, location, temperature, duration, savefreq, stepsize, default_torsions=None,
                 dihedral_pickle_file=None, dihedrals=None, metad_bias_file=None):
        ...
        if metad_bias_file is not None:
            ...
            assert len(self.metad_weights) == len(
                self.trajectory), f"metadynamics weights (len {len(self.metad_weights)}) and trajectory (len {len(self.trajectory)}) must have the same length."
        else:
            self.metad_weights = None
        self.conformations = len(self.trajectory)
        assert np.abs(self.conformations - int(
            self.duration / self.savefreq)) <= 1, f"duration ({duration}) and savefreq ({savefreq}) incompatible with number of conformations found in trajectory (got {self.conformations}, expected {int(self.duration / self.savefreq)})."
        ...
        self.dmap = dfm.DiffusionMap.from_sklearn(epsilon='bgh', alpha=0.5, k=64, kernel_type='gaussian', n_evecs=5,
                                                  neighbor_params=None,
                                                  metric='euclidean', metric_params=None, weight_fxn=None,
                                                  density_fxn=None, bandwidth_type="-1/(d+2)",
    ...
    def initialise_featurizer(self, default_torsions):
        ...
        dihedral_indices = features.create_torsions_list(atoms=self.topology.n_atoms, size=0,
                                                         append_to=default_torsions, print_list=False)
        featurizer.add_dihedrals(dihedral_indices, cossin=True)
        featurizer.describe()
        return featurizer

    def ramachandran_plot(self, data_fraction=1, bins=100, nan_threshold=50, rotate=False, low_threshold=None,
                          save_fig=False, save_name='ramachandran_plot.pdf'):
        if self.metad_weights is None:
            dihedral_traj = check_and_remove_nans(self.dihedral_traj)
            final_iteration = int(data_fraction * len(dihedral_traj))
            free_energy, xedges, yedges = free_energy_estimate_2D(dihedral_traj[:final_iteration], self.beta, bins=bins,
                                                                  weights=self._slice(self.metad_weights,
                                                                                      final_iteration))
            fig, ax = plt.subplots()
            if rotate is True:
                free_energy = free_energy.T
