...
class TMDmap_Explicit_Weights(DiffusionMap):
    """
    ...
    def __init__(
        ...
        """
        super().__init__(self, alpha, k, epsilon, metric, metric_params)
        self.kernel_type = kernel_type
        self.n_evecs = n_evecs
        self.neighbor_params = neighbor_params
        self.epsilon_fitted = None
        self.d = None
        self.explicit_dentity = explicit_dentity
    ...
    def _make_right_norm_vec(self, kernel_matrix, weights=None):
        if self.explicit_dentity is None:
            # perform kde
            q = np.array(kernel_matrix.sum(axis=1)).ravel()
        else:
            q = self.explicit_dentity
