...
class LSDMap(DiffusionMap):
    ...
    def __init__(
        ...
        assert metric in [
            ...
            "rmsd",
            "cmd",
            "contact_matrix_distance",
            "dihedral",
        ], f"Metric '{metric}' is invalid"
        assert isinstance(
            epsilon, float
        ), f"Epsilon must be a float, not {type(epsilon)}"
        self.status = status

    def _convert_PDB_trajectory_to_gro_trajectory(self):
        pdb_to_gro_command = (
            f"gmx pdb2gmx -f {self.trajectory_file}.pdb  -o {self.trajectory_file}.gro"
        )
    ...
    def _construct_config_file(self, file_name):
        with open(file_name, "w") as f:
            ...
            if self.metric_params is not None:
                assert (
                    "r0" in self.metric_params.keys()
                ), "The metric parameter r0 was not supplied"
    ...
    def _check_pdb_file(self, pdb_file):
        ...
        assert (
            num_lines > 2
        ), f"Check your PDB experiment file ({self.trajectory_file}) - it appears to be empty!"
