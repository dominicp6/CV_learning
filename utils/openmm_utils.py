...
class EquilibrationProtocol:
    def __init__(self,
                 ...
                 system_objs: SystemObjs,
                 simulation_props: SimulationProps,
                 fixed_density: bool = True):

        self.system_args = system_args
        self.system_objs = system_objs
        self.simulation_props = simulation_props
        ...
        if production_ensemble == "NVE":
            ...
            self.simulation_sequence = (("NVT", "1ns"), ("NVE", "1ns"))
        elif production_ensemble == "NVT":
            if fixed_density:
                # 1. Short NVT simulation to relax to temperature of interest
                self.simulation_sequence = (("NVT", "1ns"),)
            else:
                # 1. Short NVT simulation to relax to temperature of interest
                ...
                self.simulation_sequence = (("NVT", "1ns"), ("NPT", "1ns"), ("NPT", "1ns"), ("NVT", "1ns"))
        elif production_ensemble == "NPT":
            # 1. Short NVT simulation to relax to temperature of interest
            # 2. Short NPT simulation to relax to density of interest
            self.simulation_sequence = (("NVT", "1ns"), ("NPT", "1ns"))
        else:
    ...
    def _run_simulation(self, simulation_type: str, duration: str):
...
class OpenMMSimulation:
    ...
    def __init__(self):
        ...
        self.valid_force_fields = ["amber", "charmm"]
    ...
    def _init_simulation(self):
        ...
        """
        properties = {
            "CudaDeviceIndex": self.args.gpu,
            "Precision": self.args.precision,
        }

        unit_cell_dims = self._get_unit_cell_dims()
        ...
        if self.args.pressure is None:
            ...
            self._add_barostat(barostat_type="MonteCarloBarostat")
            print(f"Running NPT simulation at temperature {self.args.temperature} and pressure {self.args.pressure}.")

        integrator = self._get_integrator(integrator_type="LangevinIntegrator")
    ...
    def _get_unit_cell_dims(self):
        unit_cell_dims = self.system.modeller.getTopology().getUnitCellDimensions()
        if self.args.periodic:
            print(f"Unit cell dimensions: {unit_cell_dims}, Cutoff distance: {self.args.nonbondedcutoff}.")
        return unit_cell_dims

    def _add_barostat(self, barostat_type="MonteCarloBarostat"):
        if barostat_type == "MonteCarloBarostat":
            # default frequency for pressure changes is 25 time steps
            barostat = openmm.MonteCarloBarostat(
                self.args.pressure, self.args.temperature, 25,
            )
        else:
            raise NotImplementedError(f"Barostat type {barostat_type} not implemented.")

        self.system.system.addForce(barostat)

    def _get_integrator(self, integrator_type="LangevinIntegrator"):
        if integrator_type == "LangevinIntegrator":
            integrator = openmm.LangevinMiddleIntegrator(
                self.args.temperature,
                self.args.frictioncoeff,
                self.args.stepsize,
            )
        else:
